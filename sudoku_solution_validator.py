...
def validSolution(board):
    ...
    return not filter(
        lambda x: set(x) != set(range(1, 10)), board + zip(*board) + blocks
    )
...
def valid_solution(m):
    ...
    get_block_as_row = lambda n: [
        m[3 * (n / 3) + (i / 3)][(3 * n) % 9 + i % 3] for i in range(9)
    ]
    return (
        all([is_valid(r) for r in m])
        and all([is_valid([r[i] for r in m]) for i in range(9)])
        and all([is_valid(get_block_as_row(i)) for i in range(9)])
    )
...
def validSolution(board):
    ...
    blocks = [
        [board[i][j] for (i, j) in product(xrange(x, x + 3), xrange(y, y + 3))]
        for (x, y) in product((0, 3, 6), repeat=2)
    ]

    return all(
        sorted(line) == range(1, 10)
        for lines in (rows, columns, blocks)
        for line in lines
    )
